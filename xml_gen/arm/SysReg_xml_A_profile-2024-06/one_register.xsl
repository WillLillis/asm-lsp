...
<xsl:template name="generate_footer">
  ...
  <p class="copyconf">Copyright &#169; 2010-2024 Arm Limited or its affiliates. All rights reserved. This document is Non-Confidential.</p>
</xsl:template>
...
<xsl:template match="register|register_block">
  ...
  <!-- Note that presently we don't apply templates for access_mechanisms/access_mechanism
       when a reg_address element is present. This ight want to change in future.-->
  <xsl:choose>
...
<xsl:template name="offset_accessibility">
  ...
  <xsl:choose>
    <xsl:when test="count($reg_address/reg_access/reg_access_state) = 1 and $reg_address/reg_access/reg_access_state/reg_access_level"> <!-- conditional single access -->
    ...
    <xsl:when test="count($reg_address/reg_access/reg_access_state) > 1"> <!-- multiple accesses -->
    ...
    <xsl:otherwise><p>Accesses on this interface <xsl:apply-templates select="$reg_address/reg_access/reg_access_state/reg_access_type"/>.</p></xsl:otherwise> <!-- unconditional single access -->
  </xsl:choose>
</xsl:template>
  ...
  <xsl:choose>
    <xsl:when test="@length &gt; 64">
      <table class="regdiagram">
        <xsl:attribute name="id"><xsl:value-of select="../fields/@id"/></xsl:attribute>
        <xsl:call-template name="displayArbitraryDiagram">
...
<xsl:template match="fields">
  ...
  <xsl:if test="text_after_fields != ''">
    <xsl:choose>
      <xsl:when test="fields_instance">
        <xsl:element name="h4">
          <xsl:text>Additional information for the </xsl:text>
	      <xsl:value-of select="../../field_name"/>
          <xsl:text> encoding for </xsl:text>
          <xsl:value-of select="fields_instance"/>
        </xsl:element>
      </xsl:when>
      <xsl:when test="../../field_name">
        <xsl:element name="h4">
          <xsl:text>Additional information for this </xsl:text>
	      <xsl:value-of select="../../field_name"/>
          <xsl:text> encoding</xsl:text>
        </xsl:element>
      </xsl:when>
      <xsl:otherwise>
        <xsl:element name="h4">Additional information</xsl:element>
      </xsl:otherwise>
    </xsl:choose>
...
<xsl:template name="fields_descs">
  <xsl:for-each select="field[not(./@is_expansion = 'True')]">
        ...
        <xsl:choose>
          <xsl:when test="field_name"> <!-- If a field name is defined -->
            <xsl:choose>
              ...
              <xsl:when test="(not(field_array_indexes) and not(contains(./rel_range, ',') or contains(./rel_range, ':'))) or field_array_indexes/@element_size = 1">
    ...
    <h5>
        ...
        <xsl:variable name="FIELDAT_ID" select="@id"/>
        <xsl:variable name="FIELDSET_ID" select="../@id"/>
        <xsl:variable name="CONTAINER" select="../../reg_fieldset/fieldat[@msb = $MSB and starts-with(@id, $FIELDSET_ID)]"/>
        ...
        <xsl:choose>
            <xsl:when test="not(contains($anchor_bits, ',') or contains($anchor_bits, ':'))">
...
<xsl:template match="field_resets">
    ...
    <xsl:for-each select="field_reset[(@reset_type)]">
        ...
        <xsl:variable name="RESET_INTRO">
          <xsl:choose>
            <xsl:when test="contains('AEIOUaeiou', substring(@reset_type,1,1))">
              <xsl:text>On an </xsl:text>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:choose>
            <xsl:when test="field_reset_conditions | field_instance_resets">
                <li>
                <xsl:if test="$LIST &gt; '1'">
                    <xsl:value-of select="$RESET_INTRO"/><xsl:value-of select="@reset_type"/><xsl:text> reset:</xsl:text>
                </xsl:if>
                <xsl:choose>
...
<xsl:template match="field_values">
  <xsl:if test="./@impdef = 'True'">
    <p><xsl:text> The value of this field is an </xsl:text><span class="arm-defined-word">IMPLEMENTATION DEFINED</span><xsl:text> choice of:</xsl:text></p>
  </xsl:if>
  <xsl:if test="./field_value_instance">

    <!-- Get list of all field names that are linked to, if any -->
    <xsl:variable name="all_linked_field_names">
      <xsl:for-each select="field_value_instance/field_value_links_to">
        <th><xsl:value-of select="@linked_field_name"/></th>
      </xsl:for-each>
    </xsl:variable>

    <!-- Remove duplicates from field names list -->
    <xsl:variable name="linked_field_names">
      <xsl:copy-of select="ext:node-set($all_linked_field_names)/*[not(text() = preceding-sibling::*/text())]"/>
    </xsl:variable>
    ...
    <table class="valuetable">
      <tr>
        ...
        <th>Meaning</th>
        <!-- Copy headers directly from linked_field_names set -->
        <xsl:copy-of select="ext:node-set($linked_field_names)"/>
      ...
      <xsl:for-each select="field_value_instance">
        <tr>
          ...
          <!-- Store current set of field_value_links_to -->
          <xsl:variable name="current_instance_links" select="./field_value_links_to"/>

          <!-- Iterate through columns, so we can have an entry empty if this value doesn't link to this column -->
          <xsl:for-each select="ext:node-set($linked_field_names)/*">
            <xsl:variable name="field_name" select="text()"/>
            <td>
              <!-- Find all links that match the current column -->
              <xsl:for-each select="ext:node-set($current_instance_links)[@linked_field_name = $field_name]">
                <a>
                  <!-- Link target to a partial_fieldset instance -->
                  <xsl:attribute name="href">#<xsl:value-of select="@linked_field_id"/></xsl:attribute>
                  <!-- Link text -->
                  <xsl:value-of select="@linked_field_name"/>
                  <xsl:text> encoding for </xsl:text>
                  <xsl:value-of select="@linked_field_condition"/>
                </a>
              </xsl:for-each>
            </td>
          </xsl:for-each>
...
<xsl:template match="valid_values">
  <p>
  The table above shows all possible values of this field.
  <xsl:choose>
  <xsl:when test="count(valid_value) > 1">
  The values this field can take in the current implementation are:
  </xsl:when>
  <xsl:otherwise>
  <span class="binarynumber"><xsl:value-of select="valid_value"/></span> is the value of this field in the current implementation.
  </xsl:otherwise>
  </xsl:choose>
  </p>
  <xsl:if test="count(valid_value) > 1">
  <ul>
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <p>
        ...
        <xsl:if test="count(field_access_state) &lt;= 1">.</xsl:if>
      </p>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>
...
<xsl:template match="field_access_level">
    ...
    <xsl:choose>
        <xsl:when test="count(../field_access_type/field_access_state) > 0">
          <xsl:if test="count(field_access_sublevel) > 0">
            <xsl:text>When </xsl:text>
            <xsl:choose>
            ...
            <xsl:text>, access to this field is as follows:</xsl:text>
          </xsl:if>
        </xsl:when>
        <xsl:when test="count(field_access_sublevel) > 0">
            ...
            Access <xsl:apply-templates select="$access_value"/> if
                </xsl:when>
                <xsl:otherwise>
            access <xsl:apply-templates select="$access_value"/> if
                </xsl:otherwise>
        ...
        <!-- ALSO NEED TO ADD NESTING CONDITION HERE -->
            <xsl:value-of select="."/>, access to this field <xsl:apply-templates select="../field_access_type"/>.
        </xsl:otherwise>
    </xsl:choose>
</xsl:template>
...
<xsl:template match="field_access_type">
  <xsl:choose>
    ...
    <xsl:otherwise>is <span class="access_level"><xsl:apply-templates/></span></xsl:otherwise>
</xsl:choose>
</xsl:template>
...
<xsl:template match="access_mechanisms">
  ...
  <xsl:if test="access_mechanism">
    ...
    <xsl:choose>
        <xsl:when test="access_mechanism/@type = 'MemoryMapped'"> <!--PLACEHOLDER - NOT USED. RegAddress currently handles MemoryMapped.-->
            <p><xsl:value-of select="../reg_short_name"/> can be accessed through the memory-mapped interfaces:</p>
        </xsl:when>
        <xsl:when test="access_mechanism/@type = 'ExternalDebug'"> <!--PLACEHOLDER - NOT USED. RegAddress currently handles ExternalDebug.-->
            <p><xsl:value-of select="../reg_short_name"/> can be accessed through the external debug interface:</p>
        </xsl:when>
        <xsl:when test="access_mechanism/@type = 'SystemAccessor'">
        ...
        <xsl:when test="access_mechanism/@type = 'BlockAccessAbstract'">
            <!--We don't want to render a unique leading sentence for BlockAccessAbstract.-->
            <!--That may change later on, and this sentence will need to change as a result.-->
    ...
    <xsl:if test="access_condition">
     <h4 class="condition"><xsl:value-of select="access_condition"/></h4>
    </xsl:if>
    <xsl:choose>
    <xsl:when test="@type = 'CodeAbstract'">
      <p class="assembler">
        <xsl:apply-templates select="access_header"/>
      </p>
    </xsl:when>
...
<xsl:template match="xref">
  <span class="xref"><xsl:value-of select="./text()"/></span>
</xsl:template>
